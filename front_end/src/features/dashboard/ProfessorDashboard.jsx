...
export default function ProfessorDashboard({ user }) {
  ...
  const s = stats || {
    cours: { total: 0, valides: 0, en_attente: 0, rejetes: 0 },
    evaluations: { total: 0, moyenne: 0 },
  };
  ...
  return (
    <div
      className="min-h-screen  pb-12 text-[#0b1c30] font-[Inter,sans-serif]"
      ...
      <div className="max-w-[1120px] mx-auto px-4 sm:p-6 space-y-6 sm:space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base text-[#3c4a42] mt-1">
              Bonjour {user?.prenom}, gérez vos cours de révision sur
              UniConnect.
            </p>
          </div>
          <Link
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {statCards.map((card) => (
            <div
              key={card.label}
              className="bg-white p-4 sm:p-5 rounded-xl shadow border border-[#bbcabf]/30"
            >
              <div
                className={`w-10 h-10 ${card.bg} ${card.color} rounded-lg flex items-center justify-center mb-3`}
              >
                <card.icon size={18} />
              </div>
        ...
        <section className="bg-white rounded-xl shadow border border-[#bbcabf]/30">
          <div className="p-4 sm:p-5 border-b border-[#bbcabf]/30 flex justify-between items-center">
            ...
            <span className="text-sm text-[#3c4a42] font-medium">
              {cours.length} cours
            </span>
          </div>
            ...
            {loading ? (
              ...
              cours.map((c) => {
                ...
                return (
                  <div
                    key={c.id}
                    className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-xl bg-[#f8f9ff] border border-[#bbcabf]/40 hover:border-[#006c49] transition-colors"
                  >
